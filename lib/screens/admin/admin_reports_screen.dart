...
class AdminReportsScreen extends StatelessWidget {
  ...
  Future<void> _downloadSalesReport() async {
    ...
    // FIX: Use ListToCsv from the package
    String csv = const ListToCsv().convert(rows);
    await _saveFile(csv, 'sales_report.csv');
  }

  Future<void> _downloadProductReport() async {
    final snapshot = await FirebaseFirestore.instance.collection('products').get();
    final rows = [['Product ID', 'Name', 'Price', 'Stock', 'Category']];
    for (var doc in snapshot.docs) {
      final d = doc.data();
      rows.add([doc.id, d['name'], d['price'].toString(), d['stock'].toString(), d['category']]);
    }
    // FIX: Use ListToCsv from the package
    String csv = const ListToCsv().convert(rows);
  ...
  Future<void> _saveFile(String content, String filename) async {
    ...
    // Note: In a real app, you would use share_plus or open_file to show it to user.
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const TitlesTextWidget(label: "Reports")),
      body: Padding(
        ...
        child: Column(
          children: [
            ListTile(
              ...
              subtitle: const Text("Download all orders and payments"),
